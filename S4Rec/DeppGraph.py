...
class _UserModel(nn.Module):
    ...
    def forward(self, uids, iids, u_item_pad, u_user_pad, u_user_item_pad, sf_user_pad, sf_user_item_pad):
        ...
        h_sfI = self.aggre_items_sf1(h_sfI_temp.view(-1, self.emb_dim)).view(
...
class _ItemModel(nn.Module):
    ...
    def forward(self, uids, iids, i_user_pad, i_friends_pad, i_friends_user_pad):
        ...
        x_ia_s = self.g_u(torch.cat([q_a_s, u_user_item_er], dim=3).view(-1, 2 * self.emb_dim)).view(
            q_a_s.size())  # B x maxu_len x maxi_len x emb_dim

        p_i_s = mask_s.unsqueeze(3).expand_as(x_ia_s) * q_a.unsqueeze(2).expand_as(
            x_ia_s)  # B x maxu_len x maxi_len x emb_dim
        alpha_s = self.item_users_att(
            torch.cat([self.w3(x_ia_s), self.w3(p_i_s)], dim=3).view(-1, 2 * self.emb_dim)).view(
            mask_s.size())  # B x maxu_len x maxi_len
        alpha_s = torch.exp(alpha_s) * mask_s
        alpha_s = alpha_s / (torch.sum(alpha_s, 2).unsqueeze(2).expand_as(alpha_s) + self.eps)
        ...
        ## calculate attention scores in social aggregation
        alpha_i = self.if_friends_att(torch.cat([self.w4(h_oI), self.w4(p_i)], dim=2).view(-1, 2 * self.emb_dim)).view(
            mask_u.size())  # B x maxi_len
...
class DeppGraph(nn.Module):
    ...
    def perturbed_LightGCN_encoder(self, emb, adj, n_layers, eps):
        ...
        for k in range(n_layers):
            ...
            all_embs.append(emb)
        all_embs = torch.stack(all_embs).mean(dim=0)
    ...
    def create_joint_sparse_adj_tensor(self):
        csr_matrix = self.create_joint_sparse_adjaceny()
        # 将CSR格式的矩阵转换为COO格式
        norm_adj = csr_matrix.tocoo()

        indices = torch.tensor(np.vstack((norm_adj.row, norm_adj.col)).astype(np.int64), dtype=torch.long,
                               device=self.device)
        values = torch.tensor(norm_adj.data, dtype=torch.float32, device=self.device)
        size = torch.Size(norm_adj.shape)
        adj_tensor = torch.sparse_coo_tensor(indices, values, size, device=self.device)
        return adj_tensor

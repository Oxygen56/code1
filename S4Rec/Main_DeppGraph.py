...
def main():
    ...
    train_data = GRDataset(train_set, u_items_list, u_users_list, u_users_items_list, i_users_list, sf_list,
                           sf_user_item_list, i_item_friend_list, if_item_users_list, neg_transh_train_set)
    valid_data = GRDataset(valid_set, u_items_list, u_users_list, u_users_items_list, i_users_list, sf_list,
                           sf_user_item_list, i_item_friend_list, if_item_users_list, [])
    test_data = GRDataset(test_set, u_items_list, u_users_list, u_users_items_list, i_users_list, sf_list,
                          sf_user_item_list, i_item_friend_list, if_item_users_list, [])
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        valid_loss, test_loss = trainForEpoch(train_loader, valid_loader, test_loader, model, optimizer, epoch,
                                              args.epoch, criterion, best_mae, log_aggr=100)
...
def trainForEpoch(train_loader, valid_loader, test_loader, model, optimizer, epoch, num_epochs, criterion, best_mae,
    ...
    start = time.time()
    for i, (uids, iids, labels, u_items, u_users, u_users_items, i_users, i_sf_users, i_sf_users_items, i_friend_list,
            if_item_users, pos_list, neg_list) in tqdm(enumerate(train_loader), total=len(train_loader)):
        uids = uids.to(device)
        iids = iids.to(device)
...
def validate(valid_loader, model):
    ...
    results = []
    with torch.no_grad():
        for uids, iids, labels, u_items, u_users, u_users_items, i_users, i_sf_users, i_sf_users_items, i_friend_list, if_item_users, pos_list, neg_list in tqdm(
                valid_loader):
            uids = uids.to(device)
            iids = iids.to(device)
            labels = labels.to(device)
